...
// Independent, self-contained examine learning system
...
class ExamineMode extends LearningModeBase {
    ...
    async init() {
        try {
            ...
            console.error('❌ Failed to initialize examine mode:', error);
            throw error;
        }
    }

    /**
     * Render examine interface
     */
    render() {
        if (!this.container) {
        ...
        // Create main examine interface
        const examineInterface = this.createElement('div', ['examine-mode-interface'], {}, `
            ${this.renderProgressSection()}
            <div class="examine-container-wrapper">
                ${this.renderExamineCard()}
            </div>
            ${this.renderControls()}
        `);

        this.appendToContainer(examineInterface);

        // Setup examine-specific event listeners
        this.setupExamineEvents();
    ...
    renderProgressSection() {
        ...
        return `
            <div class="examine-progress-section">
                <div class="examine-progress-info">
                    <div class="progress-text">${progressText}</div>
                    <div class="category-text">${categoryText}</div>
                </div>
                <div class="examine-progress-bar">
                    <div class="examine-progress-fill" style="width: ${progress}%"></div>
                </div>
    ...
    renderFeedback() {
        ...
        return `
            <div class="examine-feedback ${isCorrect ? 'correct' : 'incorrect'}">
    ...
    renderControls() {
        ...
        return `
            <div class="examine-controls">
                <button class="examine-control-btn"
                ...
                <button class="examine-control-btn hint-btn"
                        data-action="hint"
                ...
                <button class="examine-control-btn next-btn"
                        data-action="next"
    ...
    setupExamineEvents() {
        ...
        this.container.addEventListener('click', (event) => {
            ...
            } else if (flashcard && !this.isFlipped) {
                this.flipCard();
            }
        });
    }
        ...
        return `
            <div style="text-align: right; line-height: 1.6;">
                ...
                <ul style="list-style-type: disc; margin-right: 20px;">
                    ...
                    <li>اقلب البطاقة لإظهار الخيارات</li>
...
document.addEventListener('DOMContentLoaded', () => {
    if (window.learningModeManager) {
        window.learningModeManager.registerMode('examine', ExamineMode, {
            name: 'الاختبار',
            icon: '🧐',
            description: 'اختبر معرفتك بالكلمات مع بطاقات.',
